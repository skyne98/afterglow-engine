  ...
  if (d?.cmd === 'boot') {
    void (async () => {
      try {
        ...
        wasmMemory = new WebAssembly.Memory({ initial: 128, maximum: 4096, shared: true });
        const { instance } = await WebAssembly.instantiate(bytes, { env: { memory: wasmMemory } });
        ...
        const text = `tile pool worker ${d.workerId} failed: ${(err as Error)?.message ?? err}`;
        (self as unknown as Worker).postMessage({ type: 'bootFailed', text });
      }
      return;
    })();
    return;
