...
function resetDoc(w: number, h: number) { if (w < 64 || h < 64 || w > 16384 || h > 16384) return; clearPointerInput(); docSize.width = w; docSize.height = h; const r = Math.max(w, h) / 4096; const ds = r <= 1 ? 1 : r <= 2 ? 2 : 4; dispW = Math.ceil(w / ds); dispH = Math.ceil(h / ds); canvas.style.width = `${dispW}px`; canvas.style.height = `${dispH}px`; send({ cmd: 'init', width: w, height: h, hardwareConcurrency: navigator.hardwareConcurrency }); }
...
async function init() { statusEl.textContent = 'Loading brush engine…'; log('loading brush engine…');
  const off = canvas.transferControlToOffscreen(); send({ cmd: 'init', width: docSize.width, height: docSize.height, canvas: off, hardwareConcurrency: navigator.hardwareConcurrency }, [off]);
