...
fn build_oracle() -> std::path::PathBuf {
    ...
    let src_mtime = |p: std::path::PathBuf| std::fs::metadata(p).and_then(|m| m.modified()).ok();
...
fn compositor_matches_mypaint_reference() {
    ...
    const LEVELS: [u16; 9] = [0, 4096, 8192, 12288, 16384, 20480, 24576, 28672, 32768];
    const ALPHAS: [u16; 6] = [0, 4096, 8192, 16384, 24576, 32768];
    const OPACS: [f32; 4] = [0.25, 0.5, 0.75, 1.0];
    const TRI: [u16; 27] = [
        0, 0, 0, 32768, 0, 0, 32768, 32768, 0, 0, 32768, 0, 0, 0, 32768, 32768, 0, 32768, 16384,
        8192, 24576, 8192, 24576, 16384, 24576, 16384, 8192,
    ];
    let ntri = TRI.len() / 3;
    ...
    let cases_file =
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/compositor-cases.bin");
    let out_file =
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/compositor-ref.bin");
                    ...
                    for sl in 0..9usize {
                        for dl in 0..9usize {
                            for oi in 0..4usize {
                                ...
                                stdin.write_all(&rec).expect("write oracle case");
                                count += 1;
                                count_check += 1;
                            }
                        }
                ...
                for sl in 0..9usize {
                    for dl in 0..9usize {
                        for oi in 0..4usize {
                            ...
                            for c in 0..4 {
                                let refp =
                                    u16::from_le_bytes([raw[off + c * 2], raw[off + c * 2 + 1]]);
