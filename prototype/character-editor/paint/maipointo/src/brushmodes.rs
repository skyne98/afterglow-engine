...
/// `get_color_pixels_accumulate` with the spectral `paint` path deferred:
/// `paint < 0` falls back to legacy sampling (as in C); `paint == 0` samples
/// additively with interval/random skipping; `paint > 0` panics until the
/// spectral port lands.
pub fn get_color_accumulate(
    ...
    debug_assert!(paint <= 0.0, "spectral paint path is deferred");
    ...
    let random_sample_threshold = (random_sample_rate * rand_max as f32) as i32;
    for_each_masked_pixel!(mask, mi, pi, {
        let p = pi * 4;
