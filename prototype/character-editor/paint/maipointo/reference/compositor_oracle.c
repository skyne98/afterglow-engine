/* Extracted reference (DATA) from layer-compositor.parity.test.c for the
 * maipointo Rust compositor parity oracle. GPL-2.0-or-later; not linked
 * into the engine. */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: compositor-oracle <cases.bin> <out.bin>\n");
        return 2;
    }
    ...
    uint32_t opac;
    while (fread(&mode, 2, 1, in) == 1) {
        if (fread(src, 2, 4, in) != 4) return 2;
        if (fread(dst, 2, 4, in) != 4) return 2;
        if (fread(&opac, 4, 1, in) != 1) return 2;
        ref_combine((int)mode, src, dst, opac);
        fwrite(dst, 2, 4, out);
    }
    fclose(in);
    fclose(out);
