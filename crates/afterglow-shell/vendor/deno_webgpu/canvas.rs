            ...
            let texture = match texture_descriptor {
                Descriptor::Texture(texture_descriptor) => {
                    let (id, err) =
                        device
                            .instance
                            .device_create_texture(device.id, texture_descriptor, None);
                ...
                Descriptor::Surface(surface) => {
                    ...
                    let mut output = configuration
                        .device
                        .instance
                        .surface_get_current_texture(surface_data.id, None)
                        .map_err(|e| JsErrorBox::generic(e.to_string()))?;

                    // Browser WebGPU absorbs transient native surface changes.
                    // wgpu exposes them as Outdated/Lost, so reconfigure from
                    // the current canvas descriptor and retry one acquisition.
                    if matches!(output.status, SurfaceStatus::Outdated | SurfaceStatus::Lost) {
                        let err = configuration.device.instance.surface_configure(
                            surface_data.id,
                            configuration.device.id,
                            surface,
...
impl GPUCanvasContext {
        ...
        match &self.data {
            ...
            ContextData::Surface(surface_data) => {
                ...
                Ok(Descriptor::Surface(SurfaceConfiguration {
                    ...
                    // Native game windows are composited as opaque surfaces. JavaScript
                    // still renders with WebGPU's premultiplied canvas semantics, while
                    // the final presentation discards destination alpha; requesting a
                    // platform PreMultiplied swapchain fails on opaque-only surfaces
                    // such as X11 and is not required for an opaque window.
    ...
    pub fn copy_image_contents_to_canvas_data(
        ...
        if let Some(texture) = self.current_texture.borrow().as_ref() {
            ...
            let underlying_texture =
                deno_core::cppgc::try_unwrap_cppgc_object::<GPUTexture>(scope, local).unwrap();
            ...
            image.replace_with(|image| {
                let (width, height) = image.dimensions();
                let image = deno_image::image::RgbaImage::from_raw(width, height, data).unwrap();
                DynamicImage::from(image)
            });
        }
    ...
    fn expire_current_texture(&self, scope: &mut v8::PinScope<'_, '_>) {
        if self.current_texture.borrow().is_none() {
            return;
        }
        match &self.data {
            ContextData::Canvas(_) => {
                let texture = self.current_texture.borrow();
                let local = v8::Local::new(scope, texture.as_ref().unwrap()).cast::<v8::Value>();
                let underlying_texture =
                    deno_core::cppgc::try_unwrap_cppgc_object::<GPUTexture>(scope, local).unwrap();
                underlying_texture
                    .instance
            ...
            ContextData::Surface(surface_data) => {
                ...
                surface_data
                    .instance
                    .surface_texture_discard(surface_data.id)
                    .expect("discard current WebGPU surface texture");
            }
        }
    }

    /// Present the current surface texture after JavaScript has submitted its
    /// frame. Headless canvas contexts return `Ok(false)` and remain available
    /// for explicit readback.
    pub fn present(&self) -> Result<bool, JsErrorBox> {
        let ContextData::Surface(surface_data) = &self.data else {
            return Ok(false);
        };
        if self.current_texture.borrow().is_none() {
        ...
        surface_data
            .instance
            .surface_present(surface_data.id)
    ...
    pub fn resize(&self, scope: &mut v8::PinScope<'_, '_>) {
        ...
        if let Some(configuration) = self.configuration.borrow().as_ref() {
            self.texture_descriptor.replace(Some(
                self.get_descriptor_for_configuration(configuration)
                    .unwrap(),
            ));
            ...
            match &self.data {
                ...
                ContextData::Surface(surface_data) => {
                    ...
                    let Descriptor::Surface(descriptor) = texture_descriptor.as_ref().unwrap()
                    else {
                        unreachable!()
                    };
    ...
    pub fn bitmap_read_hook(&self, scope: &mut v8::PinScope<'_, '_>) -> Result<(), JsErrorBox> {
        self.copy_image_contents_to_canvas_data(scope)
    }

    pub fn post_transfer_to_image_bitmap_hook(&self, scope: &mut v8::PinScope<'_, '_>) {
        self.replace_drawing_buffer(scope);
    }
}
...
pub fn copy_texture_to_vec(
    ...
    let overflow = || JsErrorBox::range_error("Texture dimensions overflow buffer-size math");
