...
use crate::{
    ...
    ShaderSource, SurfaceTargetUnsafe, TextureDescriptor, Tlas, WriteOnly, api,
...
impl ContextWgpuCore {
    /// Wrap the same wgpu-core registry used by an embedding WebGPU runtime.
    pub unsafe fn from_shared_global(global: Arc<wgc::global::Global>) -> Self {
        Self(global)
    }

    /// Build facade handles for an embedding-owned device and queue. The core
    /// IDs remain owned by the embedding runtime and are not dropped here.
    pub unsafe fn from_shared_device(
        global: Arc<wgc::global::Global>,
        device_id: wgc::id::DeviceId,
        queue_id: wgc::id::QueueId,
        features: Features,
    ) -> (CoreDevice, CoreQueue) {
        let context = unsafe { Self::from_shared_global(global) };
    ...
    pub unsafe fn from_shared_texture(
        global: Arc<wgc::global::Global>,
        texture_id: wgc::id::TextureId,
    ) -> CoreTexture {
        CoreTexture {
            context: unsafe { Self::from_shared_global(global) },
            id: texture_id,
