...
impl Device {
    /// Wrap an embedding-owned wgpu-core device and queue without creating a
    /// second adapter/device. The embedding must keep the core handles alive.
    #[cfg(wgpu_core)]
    pub unsafe fn from_shared_core(
        ...
        let (device, queue) = unsafe {
            crate::backend::ContextWgpuCore::from_shared_device(
                global, device_id, queue_id, features,
            )
        };
        (
            Self {
