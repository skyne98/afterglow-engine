...
impl Texture {
    /// Wrap an embedding-owned wgpu-core texture without taking ownership of
    /// its core ID. The embedding must keep the texture alive.
    #[cfg(wgpu_core)]
    pub unsafe fn from_shared_core(
        global: alloc::sync::Arc<wgc::global::Global>,
        texture_id: wgc::id::TextureId,
        desc: &TextureDescriptor<'_>,
    ) -> Self {
        let texture =
            unsafe { crate::backend::ContextWgpuCore::from_shared_texture(global, texture_id) };
        Self {
            inner: texture.into(),
