...
function print(message: string): void { if (output) output.textContent += `${message}\n`; }
...
const client = await PhysicsClient.spawn({ workerWasmUrl: 'physics_worker.wasm' });
try {
  ...
  for (const count of sizes) {
    ...
    for (let iteration = 0; iteration < iterations; iteration++) {
      ...
      if (result.length === count && result.every((value, index) =>
        Math.abs(value - Math.fround((input[index] ?? 0) + roundedDt)) < 1e-6)) valid++;
    }
