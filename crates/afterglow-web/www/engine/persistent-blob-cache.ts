...
export class PersistentBlobCache {
  ...
  private constructor(
    ...
    this.stats = {
      ...
      entries: 0, bytes: 0, maxBytes, maxEntries, queuedWrites: 0,
  ...
  async put(key: string, data: Uint8Array): Promise<boolean> {
    ...
    if (data.byteLength === 0 || data.byteLength > this.maxBytes ||
        this.entries + this.queued >= this.maxEntries ||
        this.packBytes + data.byteLength > this.maxBytes) {
  ...
  private async write(job: WriteJob): Promise<void> {
    ...
    try {
      ...
      try { this.packBytes = await this.backend.size('values.pack'); } catch { /* retain last known bound */ }
    } finally {
      const elapsed = performance.now() - startedAt;
  ...
  async clear(): Promise<void> {
    if (this.writing || this.queued !== 0) throw new Error('cannot clear persistent cache while writes are pending');
