...
import { RpcTransport, concat, decodeBytes, decodeU32, encodeBytes, encodeString, encodeU32, encodeU64 } from './codec.ts';
...
export class TextureClient {
  ...
  async openSource(path: string): Promise<number> {
    const args = encodeString(path);
    const resp = await this.rpc.call(3, args);
    return decodeU32(resp, 0)[0];
  }

  async transcodeRange(source: number, offset: number, len: number, targetFormat: number): Promise<Uint8Array> {
