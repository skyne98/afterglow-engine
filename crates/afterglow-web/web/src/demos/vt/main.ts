...
import {
  FeedbackRegistrationStatus, SLOT_SIZE, VirtualMaterialBinding, VirtualTextureFeedbackCoordinator,
  createProceduralVirtualTextureStore, generateTerrainPage,
} from '../../engine/virtual-texturing/index.ts';
import { BoundedKeyboardInput, DemoInputAction } from '../../engine/input/index.ts';
import {
  BootstrapGuard, BrowserErrorCapture, FrameStepHarness, PageShutdown, TextHud,
  publishDevHarness, testRawFeedback, testUploadLocations,
...
const runtime = EngineRuntime.forScene({
  ...
  entityCapacity: 1,
  memory: {
    frameScratchBytes: 16 * 1024, renderScratchBytes: 16 * 1024,
    structuralCommands: 8, workerCompletions: 8, assetRequests: 8, vtRequests: 512,
  },
...
const store = createProceduralVirtualTextureStore(
  async (_path, request) => generateTerrainPage(request.mip, request.x, request.y, VIRTUAL_SIZE),
  rendererHost.device,
);
...
coordinator = new VirtualTextureFeedbackCoordinator(
  rendererHost.renderer, store, { renderables: 1, passes: 1, cadence: 1, scale: 0.125 },
);
...
const binding = new VirtualMaterialBinding({
  scene, camera, root: quad, mesh: quad, store, set: { albedo: entry },
  ...
  if (!input.programmatic) {
    ...
    camX = Math.max(0, Math.min(1, camX +
      ((input.isDown(DemoInputAction.OrbitRight) ? 1 : 0) -
       (input.isDown(DemoInputAction.OrbitLeft) ? 1 : 0)) * speed));
    camY = Math.max(0, Math.min(1, camY +
      ((input.isDown(DemoInputAction.ZoomIn) ? 1 : 0) -
       (input.isDown(DemoInputAction.ZoomOut) ? 1 : 0)) * speed));
    const wheel = input.consumeWheelDelta();
    if (wheel !== 0) zoom = Math.max(0.5, Math.min(VIRTUAL_SIZE, zoom * Math.exp(wheel * 0.001)));
...
publishDevHarness('__afterglowVtGpuTest', {
  ...
  async run() {
    ...
    const uploads = await testUploadLocations(
      rendererHost.device, store.atlasWidth, store.atlasHeight, SLOT_SIZE,
    );
    await rendererHost.device.queue.onSubmittedWorkDone();
    if (runtime.diagnostics.count !== 0) throw new Error('VT GPU diagnostics are not empty');
    return {
      ok: true, feedbackRuns, uploads, residencyRuns,
      resident: store.getDebugSnapshot().atlasSlotsUsed,
      virtualSize: VIRTUAL_SIZE, atlas: [store.atlasWidth, store.atlasHeight],
    };
  },
});
bootstrap.release();
