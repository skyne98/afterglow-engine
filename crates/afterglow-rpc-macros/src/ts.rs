...
pub fn generate_client(tr: &ItemTrait, is_async: bool) -> syn::Result<String> {
    ...
    for (id, item) in tr.items.iter().enumerate() {
        ...
        let body = if ret.decode_fn.is_empty() {
            ...
            vec![
                format!("  async {mname}({params}): Promise<{ret_ts}> {{", ret_ts = ret.ts),
                format!("    const args = {args_expr};"),
    ...
    let mut lines = vec![
        ...
        format!("import {{ RpcTransport, {imports} }} from './codec.js';", imports = import_list.join(", ")),
    ];

    if is_async {
        lines.push("import { AsyncWorker, asyncWorkerImports } from './async-worker.js';".to_string());
    } else {
        lines.push("import { Rpc } from './rpc.js';".to_string());
    }
    ...
    lines.push(String::new());
    lines.push(format!("  constructor(rpc: RpcTransport) {{ this.rpc = rpc; }}"));
    if !methods.is_empty() {
        lines.push(String::new());
