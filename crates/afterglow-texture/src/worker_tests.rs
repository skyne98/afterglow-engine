...
fn worker_generate_mips() {
    let (client, _) = TextureClient::spawn_worker(TextureWorker).unwrap();
    ...
    assert!(count >= 4, "should have at least 4 mip levels for 64×64, got {count}");
}
...
fn worker_downscale() {
    ...
    let result: Vec<u8> = drive(&client, client.downscale(rgba.clone(), 32, 32, 16, 16).unwrap()).unwrap();
