...
curl -sf "http://127.0.0.1:${PORT}/json/version" >/dev/null || {
  echo "no VT dungeon DevTools endpoint on 127.0.0.1:${PORT}; run DISPLAY=:0 ./scripts/run-vt-dungeon.sh first" >&2
  exit 1
}
cat >&2 <<EOF
