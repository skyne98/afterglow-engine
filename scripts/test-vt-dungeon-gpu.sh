...
for run in $(seq 1 "$RUNS");do
 ...
 RESULT="$(./target/release/latency-tool eval '(async()=>{const a=window.__afterglowVtDungeon;a.setProgrammatic(true);const scenarios=[];for(const name of ["forward","reverse","corner"]){const s=await a.runScenario(name);scenarios.push({name,pose:s.pose,resident:s.atlasSlotsUsed,pending:s.pendingPages,errors:s.errors})}const final=a.snapshot();return JSON.stringify({ok:final.textures.length===12&&final.errors.length===0&&scenarios.every(s=>s.pending===0&&s.errors.length===0),textures:final.textures.map(t=>({id:t.textureId,path:t.path,size:t.virtualSize})),scenarios})})()' 127.0.0.1:9222)"
