...
const visual = new Set(manifest.artifacts.filter((entry) => entry.role === 'visual-demo')
...
if (import.meta.main) {
  ...
  for await (const file of glob.scan({ cwd: sourceRoot, onlyFiles: true })) {
    ...
    errors.push(...importBoundaryErrors(file, await readFile(join(sourceRoot, file), 'utf8'), sourceRoot, visual));
  }
  if (errors.length) {
