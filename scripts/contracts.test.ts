...
import { validateWebContracts } from './check-web-contracts.ts';
...
async function fixture(options: {
  ...
  await writeFile(join(www, 'demo.html'), options.html ?? '<script type="module" src="./demo.js"></script>\n');
  if (options.extraJs) await writeFile(join(www, options.extraJs), 'bad();\n');
  await writeFile(join(www, 'web-artifacts.json'), JSON.stringify({
    version: 1,
    artifacts: [{
