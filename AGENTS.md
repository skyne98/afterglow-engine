...
- Keep docs/subject/ for stable, comprehensive reference notes on key dependencies and subjects: feature catalogs, module maps, full API reference tables, test-backed usage, and gotchas. These are authoritative references, not design investigations.
  Example subject notes: Lightyear API reference, Avian3d API reference, lightyear_avian3d API reference, Steamworks API reference.
- Keep docs/ROADMAP.md up to date with the current vision
- The session/matchmaking layer is EXTERNAL to the engine. The engine's netcode connection layer consumes explicit connection resources/config (`ServerAddr` or `ServerListenAddr`, `LocalIdentity`, and `NetcodeConfig`) and does not know about codes, lobbies, invites, NAT traversal, or matchmaking. Future NonSteam/Steam session tooling lives outside the engine and should hand resolved connection params to `AfterglowConnectionPlugin`.
- `PlayerId` (u64) IS the netcode `client_id`. For Steam this is the `SteamId`; for NonSteam this is `blake3(public_key)[0..8]` derived from a persistent Ed25519 keypair. Identity is cryptographically proven: Steam authenticates natively; NonSteam uses challenge-response (server sends nonce, client signs with private key, server verifies). See docs/research/player-identity-auth.md and docs/subject/session-provider.md.
...
- Server-confirmed state is a past-tick fact, never a current-frame gameplay command. A client release at tick N must remain released after any confirmation for tick < N: if the confirmation differs from prediction, Lightyear must rollback to the old tick and replay through tick N using buffered input before presentation. Old authoritative rope-attached state must never visibly resurrect a rope after the local release has been replayed. Local visual suppression is allowed only as a presentation guard while authoritative despawn catches up; it is not a substitute for rollback/replay correctness.
- Deterministic physics prediction should not trigger continuous or excessive rollbacks. Frequent rollback requests for stable deterministic Avian/Lightyear bodies indicate missing history, wrong tick alignment, non-deterministic simulation, bad canonical pose ownership, or stale predicted entities; add regression counters and fix the root cause instead of masking it with pose copies or presentation hacks.
