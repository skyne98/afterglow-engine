...
| Persistent blob store | `budgeted` | Snapshot/result buffers, OPFS/native file I/O, RPC chunks, Promises | Caller hard item/total/value/in-flight limits; native OS worker uses 512 KiB RingBuffer chunks; two checksummed generations retain the previous value |
...
| Basis transcode | `budgeted` | Codec output and postcard response vectors; public-web response ownership copy | VT admits at most 16 pages. Native workers read source ranges into preallocated 4 MiB input scratch and move independently owned op responses; public web copies reusable wasm response scratch. |
| Image/model parse + meshopt | `budgeted` | GLTF/Three objects, copied worker arguments/results, compact attributes/index buffers | Fixed model handles, pending jobs, resident bytes, and completions/poll; runtime revisions retain old LODs; rigid/skinned/morphed geometry uses attribute-aware simplification |
| Model geometry publication | `budgeted` | Copies completed LOD data into persistent Three attributes and marks GPU updates | `GeometryArena` prewarms fixed layout buckets/slots; complete revisions preflight all slots and atomically swap, retaining the old publication on failure |
| Mutable RAM texture update | `budgeted` | Caller bytes, Promise completion, WebGPU boundary | Fixed canonical/dirty/output-page capacities preflight admission; sealed dirty refresh uses persistent scratch and asynchronous source reads use fixed payload leases |
