...
- Server snapshots still provide authority/correction through Lightyear's
  ...
  from `ActionState<AfterglowAction>`. Rope attach feedback now uses an
  entity-backed `RopeLink { player_owner, box_id }` with Lightyear
  `PreSpawned`: clients pre-spawn the predicted link, and the server validates
  the same input and confirms by spawning a replicated link with the same
  deterministic hash. The derived `DistanceJoint` and rope gizmo are local
  presentation/physics effects of `RopeLink`, not replicated render assets.
  A per-player release latch, minimum observed press duration, and short
  cooldown reject repeated/stale observations of the same release.
