...
pub struct TaffyStyloStyle<T: Deref<Target = ComputedValues>> {
    ...
    /// Viewport-relative size and insets for the initial containing block.
    pub initial_containing_block: Option<(taffy::Size<taffy::Dimension>, taffy::Rect<taffy::LengthPercentageAuto>)>,
}
...
impl<T: Deref<Target = ComputedValues>> TaffyStyloStyle<T> {
    ...
    pub fn new(style: T, flags: StyleFlags) -> Self {
        Self { style, flags, initial_containing_block: None }
    }
}
...
impl<T: Deref<Target = ComputedValues>> From<TaffyStyloStyle<T>> for taffy::Style<Atom> {
    fn from(value: TaffyStyloStyle<T>) -> Self {
        ...
        if let Some((size, inset)) = value.initial_containing_block {
            style.size = size;
            style.inset = inset;
        }
        style
...
impl<T: Deref<Target = ComputedValues>> taffy::CoreStyle for TaffyStyloStyle<T> {
    ...
    fn inset(&self) -> taffy::Rect<taffy::LengthPercentageAuto> {
        if let Some((_, inset)) = self.initial_containing_block { return inset; }
    ...
    fn size(&self) -> taffy::Size<taffy::Dimension> {
        if let Some((size, _)) = self.initial_containing_block { return size; }
        let position_styles = self.style.get_position();
        taffy::Size {
