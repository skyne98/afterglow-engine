...
/// Paint a clipped physical-pixel viewport region into a region-sized target.
/// The document is translated by the negative region origin so callers can
/// update a texture subrectangle without rasterizing the entire page.
pub fn paint_scene_region(
    ...
    paint_scene_with_offset(scene, doc, scale, width, height, -(x as f64), -(y as f64));
}
