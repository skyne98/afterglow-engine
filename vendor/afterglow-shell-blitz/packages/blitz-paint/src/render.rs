...
impl ElementCx<'_, '_> {
    ...
    fn draw_image(&self, scene: &mut impl PaintScene) {
        if let Some(image) = self.element.raster_image_data() {
            ...
            let is_unscaled_canvas = self.element.canvas_data().is_some()
                && width == image.width
                && height == image.height;
            // A 1:1 browser canvas copy is a texel-preserving blit. Medium
            // image filtering can sample across texel boundaries even when no
            // scaling is requested, changing established WebGPU pixels.
            let quality = if is_unscaled_canvas {
                peniko::ImageQuality::Low
            } else {
                to_image_quality(image_rendering)
            };
