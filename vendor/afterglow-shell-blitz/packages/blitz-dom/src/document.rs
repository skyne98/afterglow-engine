...
impl BaseDocument {
    ...
    pub fn set_stylesheet_text_for_node(&mut self, node_id: usize, css: &str) {
        let sheet = self.make_stylesheet(css, Origin::Author);
    ...
    /// Return every painted node under a viewport point, front to back.
    /// Nodes already returned are made transparent while descendants and
    /// lower paint-order siblings remain eligible for the next hit.
    ...
    pub fn node_is_positioned(&self, node_id: usize) -> bool {
    ...
    /// Evaluate a CSS media query list against the same Stylo device and
    /// custom-media definitions used by this document's stylesheets.
    pub fn media_query_matches(&self, query: &str) -> bool {
        ...
        let media = MediaList::parse(&context, &mut parser);
        media.evaluate(
            self.stylist.device(),
            QuirksMode::NoQuirks,
    ...
    /// Serialize a Stylo-computed longhand value for CSSOM consumers.
    pub fn computed_property(&self, node_id: usize, property_name: &str) -> Option<String> {
