...
impl BaseDocument {
    ...
    /// Set a text-input selection with UTF-8 byte offsets from the host bridge.
    pub fn set_text_input_selection(&mut self, node_id: NodeId, anchor: usize, focus: usize) {
        if let Some(input) = self.nodes[node_id].element_data_mut().and_then(|el| el.text_input_data_mut()) {
            input.editor.driver(&mut self.font_ctx.lock().unwrap(), &mut self.layout_ctx)
    ...
    pub fn set_stylesheet_text_for_node(&mut self, node_id: NodeId, css: &str) {
        let sheet = self.make_stylesheet(css, Origin::Author);
        self.add_stylesheet_for_node(sheet, node_id);
    }

    pub fn hits(&self, x: f32, y: f32) -> Vec<HitResult> {
        let mut hits = Vec::new();
        let mut excluded = HashSet::new();
        loop {
            let Some(hit) =
                self.root_element()
                    .hit_except(x, y, self.viewport().scale_f64(), &excluded)
            else {
                break;
            };
            if !excluded.insert(hit.node_id) {
                break;
            }
            hits.push(hit);
        }
    ...
    pub fn node_is_positioned(&self, node_id: NodeId) -> bool {
    ...
    pub fn computed_property(&self, node_id: NodeId, property_name: &str) -> Option<String> {
        use style::properties::PropertyId;

        let styles = self.get_node(node_id)?.primary_styles()?;
        if let Some(custom_name) = property_name.strip_prefix("--") {
            let name = style::Atom::from(custom_name);
            let custom = styles.custom_properties();
            let value = custom
                .inherited
                .get(&name)
                .or_else(|| custom.non_inherited.get(&name))?;
            return value.as_universal().map(|value| value.css.clone());
        }
        let property = PropertyId::parse_enabled_for_all_content(property_name).ok()?;
        let longhand = property.longhand_id()?;
        let mut value = String::new();
        styles
            .computed_or_resolved_value(longhand, None, &mut value)
            .ok()?;
        Some(value)
    }
    ...
    fn client_transform(&self, node_id: NodeId) -> kurbo::Affine {
        let mut transform = kurbo::Affine::IDENTITY;
        let mut current = self.get_node(node_id);
        let scale = f64::from(self.viewport().scale());
        while let Some(node) = current {
            if let Some(matrix) = node.transform().as_deref() {
                let [a, b, c, d, e, f] = matrix.as_coeffs();
                let scroll = node.scroll_offset();
                let origin = node.absolute_position(scroll.x as f32, scroll.y as f32);
                let origin = kurbo::Vec2::new(origin.x as f64 - self.viewport_scroll.x, origin.y as f64 - self.viewport_scroll.y);
                transform = kurbo::Affine::translate(origin)
                    * kurbo::Affine::new([a, b, c, d, e / scale, f / scale])
                    * kurbo::Affine::translate(-origin) * transform;
            }
            current = node.layout_parent.get().and_then(|id| self.get_node(id));
        }
        transform
    }

    pub(crate) fn page_point_to_local(&self, node_id: NodeId, x: f32, y: f32) -> Option<(f32, f32)> {
        let transform = self.client_transform(node_id);
        if !transform.determinant().is_finite() || transform.determinant().abs() < f64::EPSILON { return None; }
        let point = transform.inverse() * kurbo::Point::new(x as f64 - self.viewport_scroll.x, y as f64 - self.viewport_scroll.y);
        let node = self.get_node(node_id)?;
        let scroll = node.scroll_offset();
        let origin = node.absolute_position(scroll.x as f32, scroll.y as f32);
        Some(((point.x + self.viewport_scroll.x) as f32 - origin.x, (point.y + self.viewport_scroll.y) as f32 - origin.y))
    }

    fn transform_client_rect(&self, node_id: NodeId, rect: BoundingRect) -> BoundingRect {
        let bounds = self.client_transform(node_id).transform_rect_bbox(kurbo::Rect::new(rect.x, rect.y, rect.x + rect.width, rect.y + rect.height));
        BoundingRect { x: bounds.x0, y: bounds.y0, width: bounds.width(), height: bounds.height() }
    }

    /// Get the text-editor IME area in viewport CSS pixels.
    pub fn text_input_cursor_rect(&self, node_id: NodeId) -> Option<BoundingRect> {
        let node = self.get_node(node_id)?;
        let input = node.element_data()?.text_input_data()?;
        let layout = node.final_layout();
        let scale = self.viewport.scale_f64();
        let area = input.editor.ime_cursor_area();
        let scroll = node.scroll_offset();
        let pos = node.absolute_position(scroll.x as f32, scroll.y as f32);
        let mut x = pos.x as f64 + layout.content_box_x() as f64 - self.viewport_scroll.x;
        let mut y = pos.y as f64 + layout.content_box_y() as f64 - self.viewport_scroll.y;
        if input.is_multiline { y -= input.scroll_offset as f64; }
        else {
            x -= input.scroll_offset as f64;
            let text = input.editor.try_layout()?;
            y += ((layout.content_box_height() - text.height() / text.scale()) / 2.0).max(0.0) as f64;
        }
        Some(self.transform_client_rect(node_id, BoundingRect {
            x: x + area.x0 / scale, y: y + area.y0 / scale,
            width: (area.x1 - area.x0) / scale, height: (area.y1 - area.y0) / scale,
        }))
    }
    ...
    pub fn get_client_bounding_rect(&self, node_id: NodeId) -> Option<BoundingRect> {
        ...
        let node = self.get_node(node_id)?;
        // Content scroll moves children, not the container border box.
        let scroll = node.scroll_offset();
        let pos = node.absolute_position(scroll.x as f32, scroll.y as f32);
    ...
    pub fn inline_fragment_rects(&self, node_id: NodeId) -> Option<Vec<BoundingRect>> {
        ...
        for line in layout.lines() {
            ...
            if let Some((x0, y0, x1, y1)) = line_rect {
                rects.push(self.transform_client_rect(inline_root.id, BoundingRect {
                    x: origin_x + x0 / scale,
                    y: origin_y + y0 / scale,
                    width: (x1 - x0) / scale,
                    height: (y1 - y0) / scale,
