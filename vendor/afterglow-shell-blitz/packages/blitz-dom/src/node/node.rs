...
impl Node {
    ...
    /// Return the topmost hit while treating the supplied nodes as transparent.
    /// Descendants remain hittable, matching `pointer-events: none` traversal.
    pub fn hit_except(
        &self,
        x: f32,
        y: f32,
    ...
    pub(crate) fn hit_inner(
        ...
        // `overflow: visible` descendants remain hit-testable outside the
        // parent's own box. Only clipped/scrolling axes constrain descent.
        let (overflow_x, overflow_y) = self
            .primary_styles()
            .map(|style| (style.clone_overflow_x(), style.clone_overflow_y()))
            .unwrap_or((
            ...
            if let Some(hoisted) = &self.stacking_context {
                for hoisted_child in hoisted.pos_z_hoisted_children().rev() {
                    ...
                    if let Some(hit) = self
                        ...
                        .hit_inner(x, y, scale, scrollbar, excluded)
                    {
                        return Some(hit);
                    }
                }
            ...
            if let Some(ild) = element_data.inline_layout_data.as_ref() {
                    ...
                    Cluster::from_point_exact(layout, x * scale, y * scale)
                {
                    ...
                    if !text_pointer_events_none
                        && !excluded.is_some_and(|nodes| nodes.contains(&node_id))
                    {
        ...
        if matches_self
            && !pointer_events_none
            && !excluded.is_some_and(|nodes| nodes.contains(&self.id))
        {
            return Some(HitResult {
                node_id: self.id,
                x,
