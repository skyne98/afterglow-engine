...
impl ElementData {
    ...
    pub fn raster_image_data(&self) -> Option<&RasterImageData> {
        match &self.special_data {
            SpecialElementData::Image(data) => match &**data {
                ...
                #[cfg(feature = "svg")]
                ImageData::Svg(_) => None,
                ImageData::None => None,
            },
            SpecialElementData::Canvas(data) => data.raster.as_ref(),
            _ => None,
        }
    }
    ...
    pub fn raster_image_data_mut(&mut self) -> Option<&mut RasterImageData> {
        match &mut self.special_data {
            SpecialElementData::Image(data) => match &mut **data {
                ImageData::Raster(data) => Some(data),
                #[cfg(feature = "svg")]
                ImageData::Svg(_) => None,
                ImageData::None => None,
            },
            SpecialElementData::Canvas(data) => data.raster.as_mut(),
            _ => None,
        }
    }
...
pub struct CanvasData {
    pub custom_paint_source_id: Option<u64>,
    /// Browser canvas pixels. Canvas remains a canvas replaced element rather
    /// than masquerading as an image; the raster is only its paint source.
    pub raster: Option<RasterImageData>,
}
